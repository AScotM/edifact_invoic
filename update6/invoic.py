...
class EDIFACTConfig:
    ...
    DEFAULT_PRECISION = 2  # Added for configurable decimal precision
    ...
    MAX_QUANTITY = 999999  # Added for quantity bounds
    MAX_PRICE = 9999999.99  # Added for price bounds
    ...
    VALID_ENCODINGS = {"utf-8", "ascii", "iso-8859-1"}  # Added for encoding validation
...
class EDIFACTGenerator:
    ...
    def generate_invoic(
        ...
        unt_segment = self._build_segment(
            ...
            str(len(segments)),  # Fixed: removed +1, as len(segments) now represents the number of segments before UNT, which equals the message count including UNT (since UNB is 1, and len = 1 + message_without_UNT, but message_count = message_without_UNT +1 = len -1 +1 = len
        ...
        expected_message_count = len(segments) - 2  # Total minus UNB and UNZ
        unt_count = int(unt_segment.split(self.data_separator)[1])
        if unt_count != expected_message_count:
            self.logger.error({"message": "Segment count mismatch", "expected": expected_message_count, "reported": unt_count})
            raise EDIFACTGeneratorError(f"Segment count mismatch: expected {expected_message_count}, reported {unt_count}")
